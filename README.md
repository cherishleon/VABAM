...
> **Note:** The visualization code was developed with GPT and Claude assistance and prioritizes functional output over readability. Some redundancy may be present, and further updates are expected.
...
### 1.Dataset
You can download the **processed dataset** and **benchmark evaluation instances** using either Git Bash / Command Prompt with `wget`, or directly via the URLs provided below.
> *Note*: Benchmark evaluation instances are originally generated by running `BatchBMMIEvaluation.py`, which can be time-consuming.
> We provide the pre-generated results from our study for users who wish to skip this step. After downloading, please extract the archive to the `./Benchmarks/EvalResults/Instances` directory.

- **Processed Dataset**: [Download ProcessedData.Egg](https://www.dropbox.com/scl/fi/vv0isaldp8pnpvvr7d59b/ProcessedData.Egg?rlkey=bjmcadwtmu3b40e3e41gltjs9&dl=1)
- **Benchmark Instances**: [Download Instances.egg](https://www.dropbox.com/scl/fi/8chxeazx0chunp1uah8l4/Instances.zip?rlkey=2a29ttvlw4d5o5vv2j94l8g83&dl=1)
