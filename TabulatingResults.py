...
from Utilities.AncillaryFunctions64 import Denorm, MAPECal, MSECal
...
def Aggregation (ConfigName, ConfigPath, NJ=1, FC=1.0, MetricCut = 1., BatSize=3000):
    ...
    print('Loading data')
    ## Loading data
    AnalData = np.load('./Data/ProcessedData/'+str(Params['TestDataSource'])+'Val'+str(Params['SigType'])+'.npy')
    ...
    with open('./Data/ProcessedData/'+str(Params['DataSource'])+'SigMax.pkl', 'rb') as f:
        SigMax = pickle.load(f)
    with open('./Data/ProcessedData/'+str(Params['DataSource'])+'SigMin.pkl', 'rb') as f:
