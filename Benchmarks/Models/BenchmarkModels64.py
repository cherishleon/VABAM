...
from Benchmarks.Models.BaseVAES64 import *
from Benchmarks.Models.Wavenet64 import *
from Benchmarks.Models.DiffWave64 import *
from Benchmarks.Models.VDiffWave64 import *
from Models.Discriminator64 import FacDiscriminator
...
def BaseVAE(SigDim, ConfigSpec,  SlidingSize = 50, Reparam=True, ReparaStd=None):
    ...
    Beta_Z   = tf.cast(Lossweight(name='Beta_Z', InitVal=0.01)(VAEModel.input), tf.float64)
    Beta_Rec = tf.cast(Lossweight(name='Beta_Rec', InitVal=1.)(VAEModel.input), tf.float64)
...
def VDVAE(SigDim, ConfigSpec,  SlidingSize = 50, Reparam=True, ReparaStd=None):
    ...
    ### Weight controller; Apply beta and capacity 
    Beta_Z   = tf.cast(Lossweight(name='Beta_Z', InitVal=0.01)(VDVModel.input), tf.float64)
    Beta_Rec = tf.cast(Lossweight(name='Beta_Rec', InitVal=1.)(VDVModel.input), tf.float64)
...
def ConVAE(SigDim, CondDim, ConfigSpec, SlidingSize = 50, Reparam=True, ReparaStd=None):
    ...
    ### Weight controller; Apply beta and capacity 
    Beta_Z   = tf.cast(Lossweight(name='Beta_Z', InitVal=1.0)(ReconOut), tf.float64)
    Beta_Rec = tf.cast(Lossweight(name='Beta_Rec', InitVal=1.0)(ReconOut), tf.float64)
...
def TCVAE(SigDim, NData, ConfigSpec, SlidingSize = 50, Reparam=True, ReparaStd=None):
    ...
    BatSize = tf.cast(ConfigSpec['BatSize'], tf.float64) 
    
    ### Weight controller; Apply beta and capacity 
    Beta_Z   = tf.cast(Lossweight(name='Beta_Z', InitVal=1.0)(ReconOut), tf.float64)
    Beta_TC  = tf.cast(Lossweight(name='Beta_TC', InitVal=1.0)(ReconOut), tf.float64)
    Beta_MI  = tf.cast(Lossweight(name='Beta_MI', InitVal=1.0)(ReconOut), tf.float64)
    Beta_Rec = tf.cast(Lossweight(name='Beta_Rec', InitVal=1.0)(ReconOut), tf.float64)
...
def FACVAE(SigDim, ConfigSpec, SlidingSize = 50, Reparam=True, ReparaStd=None):
    ...
    ### Weight controller; Apply beta and capacity 
    Beta_Z   = tf.cast(Lossweight(name='Beta_Z', InitVal=0.01)(FactorVAEModel.input), tf.float64)
    Beta_TC  = tf.cast(Lossweight(name='Beta_TC', InitVal=0.01)(FactorVAEModel.input), tf.float64)
    Beta_DTC = tf.cast(Lossweight(name='Beta_DTC', InitVal=0.01)(FactorVAEModel.input), tf.float64)
    Beta_Rec = tf.cast(Lossweight(name='Beta_Rec', InitVal=1.0)(FactorVAEModel.input), tf.float64)
